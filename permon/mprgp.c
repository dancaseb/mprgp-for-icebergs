...
int main(int argc, char **args)
{
  ...
  char        a_file[PETSC_MAX_PATH_LEN], b_file[PETSC_MAX_PATH_LEN], c_file[PETSC_MAX_PATH_LEN]; /* input file names */
  PetscBool   a_flg, b_flg, c_flg;

  PetscCall(PermonInitialize(&argc, &args, (char *)0, help));
  PetscCall(PetscOptionsGetInt(NULL, NULL, "-n", &n, NULL));
  PetscCall(PetscOptionsGetBool(NULL, NULL, "-sol", &viewSol, NULL));

  PetscCall(PetscOptionsGetString(NULL, NULL, "-a", a_file, sizeof(a_file), &a_flg));
  PetscCheck(a_flg, PETSC_COMM_WORLD, PETSC_ERR_USER, "Must indicate binary file with the -a option");

  PetscCall(PetscOptionsGetString(NULL, NULL, "-b", b_file, sizeof(b_file), &b_flg));
  PetscCheck(b_flg, PETSC_COMM_WORLD, PETSC_ERR_USER, "Must indicate binary file with the -b option");

  PetscCall(PetscOptionsGetString(NULL, NULL, "-c", c_file, sizeof(c_file), &c_flg));
  PetscCheck(c_flg, PETSC_COMM_WORLD, PETSC_ERR_USER, "Must indicate binary file with the -c option");
  ...
  *  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  PetscCall(PetscViewerBinaryOpen(PETSC_COMM_WORLD, a_file, FILE_MODE_READ, &viewer));
  PetscCall(MatCreate(PETSC_COMM_WORLD, &A));
  PetscCall(MatLoad(A, viewer));
  PetscCall(PetscViewerDestroy(&viewer));

  PetscCall(PetscViewerBinaryOpen(PETSC_COMM_WORLD, b_file, FILE_MODE_READ, &viewer));
  PetscCall(VecCreate(PETSC_COMM_WORLD, &b));
  PetscCall(VecLoad(b, viewer));
  PetscCall(PetscViewerDestroy(&viewer));

  PetscCall(PetscViewerBinaryOpen(PETSC_COMM_WORLD, c_file, FILE_MODE_READ, &viewer));
  PetscCall(VecCreate(PETSC_COMM_WORLD, &c));
  PetscCall(VecLoad(c, viewer));
  PetscCall(PetscViewerDestroy(&viewer));

  PetscCall(VecDuplicate(c, &x));
  ...
  *  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  PetscCall(PetscViewerBinaryOpen(PETSC_COMM_WORLD, "x_output.bin", FILE_MODE_WRITE, &viewer));
